...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        super().__init__()
        p = autopad(k, p, d)
        p = tuple(p) if isinstance(p, list) else p
...
class MultiStreamConv(nn.Module):
    ...
    def forward(self, x):
        assert len(x) == len(self.conv_layers), f'The number of input data stream does not match the predefined settings. len x:{len(x)}, len conv_layers: {len(self.conv_layers)}'
...
class C3(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.Sequential(*(NewBottleneck(c_, c_, shortcut, g, k=((1, 1), (3, 3)), e=1.0) for _ in range(n)))
...
class C2f(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        ...
        self.m = nn.ModuleList(NewBottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C3k2(C2f):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else NewBottleneck(self.c, self.c, shortcut, g) for _ in range(n)
        )
...
class C3k(C3):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5, k: int = 3):
        ...
        self.m = nn.Sequential(*(NewBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        int.
        """
        # assert x[0].shape[2:] == x[1].shape[2:], f'Shapes {x[0].shape[2:]} and {x[1].shape[2:]} must match for concatenation.'
