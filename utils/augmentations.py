...
def multi_random_perspective(
    ims, targets=(), segments=(), degrees=10, translate=0.1, scale=0.1, shear=10, perspective=0.0, border=(0, 0)
):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(0.1, 0.1), scale=(0.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]

    height = ims[0].shape[0] + border[0] * 2  # shape(h,w,c)
    width = ims[0].shape[1] + border[1] * 2

    # Center
    C = np.eye(3)
    C[0, 2] = -ims[0].shape[1] / 2  # x translation (pixels)
    C[1, 2] = -ims[0].shape[0] / 2  # y translation (pixels)
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        for i, im in enumerate(ims):
            if perspective:
                ims[i] = cv2.warpPerspective(im, M, dsize=(width, height), borderValue=(114, 114, 114))
            else:  # affine
                ims[i] = cv2.warpAffine(im, M[:2], dsize=(width, height), borderValue=(114, 114, 114))
        # if perspective:
